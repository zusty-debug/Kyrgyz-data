...
Roles:
  - owner    : Telegram ID set by OWNER_TELEGRAM_ID env var. Unlimited.
  - admin    : Promoted by owner. Unlimited.
  - premium  : Granted by owner/admin. Unlimited.
  - regular  : 5 searches per day.

Quota is tracked in Postgres (same DB as the API) in `bot_daily_usage`.
"""
...
# Ensure /app is importable when running as `python -m bot.bot`
...
DATABASE_URL = os.environ.get(
    "DATABASE_URL",
    "sqlite:///" + os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data", "bot_state.db"),
)
OWNER_TELEGRAM_ID = int(os.environ.get("OWNER_TELEGRAM_ID", "0"))
DEV_HANDLE = os.environ.get("DEV_HANDLE", "@xzusty")
DAILY_QUOTA = int(os.environ.get("DAILY_QUOTA", "5"))
PER_PAGE = int(os.environ.get("PER_PAGE", "5"))
MAX_RESPONSE_CHARS = 3500  # Telegram limit = 4096; keep some headroom for footer

_missing = []
if not TELEGRAM_BOT_TOKEN: _missing.append("TELEGRAM_BOT_TOKEN")
if not API_KEY: _missing.append("API_KEY")
if not OWNER_TELEGRAM_ID: _missing.append("OWNER_TELEGRAM_ID")
...
def get_or_create_user(s, tg_user):
...
def set_last_query(s, user_id: int, query: str, page: int):
...
def increment_usage(s, user_id: int):
    row = get_usage(s, user_id)
    if row is None:
...
def parse_query(qs: str) -> dict:
    """`name=Капинос dob=1955-06-03` -> {name: 'Капинос', dob: '1955-06-03'}.
    Plain text -> {name: 'qs'}."""
    parts = qs.split()
    if not parts:
        return {}
...
def call_search(params: dict, page: int = 1, limit: int = PER_PAGE) -> dict:
    ...
    r = requests.get(f"{API_URL}/api/search", params=qp, headers=headers, timeout=20)
...
HELP = (
    ...
    "  `/help`          — this help\n\n"
    "Response: JSON code block + Developer footer.\n"
    "Big results are paginated — tap `Next 5 →` to keep going (free within same search).\n\n"
    f"Developer - {DEV_HANDLE}"
)
        ...
        if not is_unlimited(user.role):
            ...
            if used >= DAILY_QUOTA:
                await update.message.reply_text(
                    ...
                    f"Try again tomorrow (resets at midnight UTC).\n\n"
                    f"Developer - {DEV_HANDLE}",
                    parse_mode=ParseMode.MARKDOWN,
                )
                return
...
def _parse_target_id(ctx) -> Optional[int]:
    if not ctx.args:
        return None
    ...
    try:
        ...
        if tid is None:
            await update.message.reply_text(
                ...
                "They must message the bot at least once first.\n\n"
                f"Developer - {DEV_HANDLE}",
                parse_mode=ParseMode.MARKDOWN,
            )
            return
...
async def post_init(app: Application):
    ...
    try:
        ...
        if u is None:
            ...
            u.role = "owner"
        s.commit()
        log.info("Bot ready. Owner=%s", OWNER_TELEGRAM_ID)
    finally:
        s.close()
